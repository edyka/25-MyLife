  ...
  const loadUserData = async (currentUser) => {
    ...
    const loadPromise = (async () => {
      ...
      try {
        ...
        const { profile, milestones, selections, settings, errors } = await dataService.getUserData(currentUser.id);

        if (errors) {
          console.warn('[Viventiva Auth] Some data failed to load:', errors);
        }

        if (profile?.data && profile.data.birth_day) {
          ...
          if (settings?.data?.settings_data) {
            ...
            if (uiSettings.pastWeekStyle !== undefined) uiStore.setPastWeekStyle(uiSettings.pastWeekStyle);
          }
