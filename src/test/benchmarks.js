...
export const benchmark = async (name, fn, iterations = 1) => {
  ...
  return {
    ...
    median,
    times,
    passed: avg <= PERFORMANCE_THRESHOLDS[name.toUpperCase().replace(/\s/g, '_')] || true,
  }
}
...
export const PERFORMANCE_TEST_SUITES = {
  'week-selection-basic': {
    ...
    tests: ['single-click-response', 'hover-animation', 'selection-state-update'],
  ...
  'paint-operations': {
    ...
    tests: ['paint-single-week', 'paint-multiple-weeks', 'clear-weeks', 'milestone-creation'],
...
export const runLoadTest = async (scenario, testFn) => {
  const { concurrentUsers, operations, duration } = LOAD_TEST_SCENARIOS[scenario]
  ...
  const userPromises = Array.from({ length: concurrentUsers }, async (_, userIndex) => {
    ...
    while (performance.now() - userStartTime < duration) {
      for (let op = 0; op < operations && performance.now() - userStartTime < duration; op++) {
  ...
  const avgOperationTime = flatResults.reduce((sum, r) => sum + r.duration, 0) / totalOperations
...
export const createPerformanceMonitor = () => {
  ...
  const checkAlerts = (name, value) => {
    ...
    if (alertThreshold && value > alertThreshold) {
      console.warn(`Performance Alert: ${name} = ${value} (threshold: ${alertThreshold})`)
