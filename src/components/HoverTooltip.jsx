...
const HoverTooltip = () => {
    const { tooltip, darkMode } = useUIStore();
    ...
    return createPortal(
        <div
            ref={tooltipRef}
            className={`fixed top-0 left-0 z-[99999] pointer-events-none px-3 py-2 rounded-lg shadow-xl border backdrop-blur-sm transition-opacity duration-200 ${darkMode
        ...
        >
            <div className="flex flex-col gap-0.5">
                {tooltip.label && (
                    <span className="font-bold text-sm">{tooltip.label}</span>
                )}
                {tooltip.content && (
