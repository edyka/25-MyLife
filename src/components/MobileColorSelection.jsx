...
const MobileColorSelection = ({
  ...
  return (
    <div
      className={`md:hidden rounded-xl shadow-lg p-4 mb-4 ${darkMode
        ? "bg-gray-800/90 border border-gray-700"
        : "bg-white/90 border border-gray-200"
        }`}
    >
      <h3
        className={`text-sm font-semibold mb-3 ${darkMode ? "text-white" : "text-gray-800"
          }`}
      >
        Paint Colors
      </h3>
        ...
        {Object.entries(colorOptions).map(([key, option]) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 p-2 rounded-lg border text-sm transition-all ${isSelected
                ? darkMode
