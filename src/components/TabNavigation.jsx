...
const TabNavigation = ({
  ...
  return (
    <>
      ...
      <nav
        className={`md:hidden w-full flex items-center justify-between px-3 py-2 border-b sticky top-0 z-30 ${
          darkMode
            ? 'premium-card-dark border-slate-700/30 backdrop-blur-lg'
            : 'premium-card border-slate-200/30 backdrop-blur-lg'
        }`}
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          {!subscriptionLoading && tier !== 'free' && (
            <div
              className={`px-1.5 py-0.5 rounded text-[8px] font-bold text-white ${currentTier.color}`}
            >
              {tier === 'life' ? '👑' : '⚡'}
            </div>
          )}
          ...
          {TABS.map(({ key, label, icon: Icon }) => {
            ...
            return (
              <button
                key={key}
                onClick={() => setCurrentTab(key)}
                className={`flex flex-col items-center justify-center px-4 py-1.5 rounded-xl transition-all duration-200 min-w-[64px] ${
                  isActive
              ...
              >
                ...
                <span
                  className={`text-[10px] font-semibold mt-0.5 ${isActive ? 'text-white' : ''}`}
                >
                  {label}
                </span>
              </button>
