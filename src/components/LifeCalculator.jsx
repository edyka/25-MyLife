...
const LifeCalculator = memo(({ darkMode, onSignUp }) => {
  ...
  return (
    <section className="relative z-10 w-full py-8 px-4 sm:px-6">
      ...
      >
        <div
          className={`rounded-[2.5rem] p-8 md:p-12 overflow-hidden relative ${darkMode ? 'bg-slate-800/50 border border-slate-700' : 'bg-white shadow-2xl border border-slate-100'}`}
        >
          {/* Background Glow */}
          <div
            className={`absolute top-0 right-0 w-64 h-64 ${theme.bgGlow} blur-[100px] rounded-full pointer-events-none`}
          />
          <div
            className={`absolute bottom-0 left-0 w-64 h-64 bg-purple-500/20 blur-[100px] rounded-full pointer-events-none`}
          />
          ...
          <div className="relative z-10 flex flex-col md:flex-row gap-12 items-center">
            ...
            <div className="flex-1 w-full text-center md:text-left">
              <div
                className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-6 ${darkMode ? `${theme.badgeBgDark} ${theme.badgeTextDark}` : `${theme.badgeBg} ${theme.badgeText}`}`}
              >
              ...
              <div className="space-y-6 max-w-xs mx-auto md:mx-0">
                ...
                <div>
                  <label
                    className={`block text-xs font-bold uppercase tracking-wider mb-1.5 ${darkMode ? 'text-slate-500' : 'text-slate-500'}`}
                  >
                    Name
                  </label>
                  <input
                    ...
                    className={`w-full px-4 py-3 rounded-xl font-medium outline-none border-2 transition-all ${theme.focusBorder} ${
                      darkMode
                ...
                <div className="flex gap-3">
                  <div className="flex-1 relative">
                    <label
                      className={`block text-xs font-medium uppercase tracking-wider mb-2 ${darkMode ? 'text-slate-500' : 'text-slate-500'}`}
                    >
                      Day
                    </label>
                    <button
                      ...
                      className={`w-full px-3 py-3.5 rounded-xl text-center font-semibold outline-none border-2 transition-all cursor-pointer ${
                        darkMode
                    ...
                    {showDayPicker && (
                      <div
                        className={`absolute bottom-full left-0 mb-1 p-3 rounded-2xl shadow-2xl z-50 w-64 ${darkMode ? 'bg-slate-800 border border-slate-700' : 'bg-white border border-slate-200'}`}
                      >
                  ...
                  <div className="flex-1">
                    ...
                    <select
                      ...
                      className={`w-full px-3 py-3.5 rounded-xl text-center font-semibold outline-none border-2 transition-all cursor-pointer ${theme.focusBorder} ${
                        darkMode
                ...
                <div
                  className={`p-4 rounded-xl border-2 ${darkMode ? 'bg-slate-900/30 border-slate-700' : 'bg-slate-50 border-slate-200'}`}
                >
                  <div className="flex justify-between items-center mb-2">
                    <label
                      className={`text-sm font-semibold ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}
                    >
                      Life Expectancy
                    </label>
            ...
            <div className="flex-1 w-full">
              <AnimatePresence mode="wait">
                {stats ? (
                  ...
                  >
                    <div
                      className={`p-6 rounded-2xl text-center ${darkMode ? 'bg-slate-900/80' : 'bg-slate-50'}`}
                    >
                  ...
                  >
                    <div
                      className={`text-center ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}
                    >
...
LifeCalculator.displayName = 'LifeCalculator'
