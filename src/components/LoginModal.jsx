...
const LoginModal = ({ isOpen, onClose, onLogin, initialMode = 'login', initialData = null }) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        if (isSignUp) {
          ...
          // Wait for the SIGNED_IN event instead of a fixed 300ms sleep, which
          // was racing on slow networks and Brave (false "Login failed" while
          // the session arrived a moment later).
          if (isDev)
            console.log('[Viventiva LoginModal] Email login successful, waiting for session...')

          const SIGNIN_TIMEOUT = 5000
          await new Promise(resolve => {
            ...
            const timeout = setTimeout(finish, SIGNIN_TIMEOUT)
          })
  ...
  // Note: we render AnimatePresence unconditionally and gate the contents on
  // `isOpen` so the exit animation can actually run. Early-returning null at
  // the top would unmount the modal before AnimatePresence could animate out.
  return createPortal(
    <AnimatePresence>
      {isOpen && (
        <motion.div
        ...
        >
          <motion.div
            ...
            className={`w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl backdrop-blur-2xl border ${
