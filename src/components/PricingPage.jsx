...
const PricingPage = ({ darkMode }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-6 sm:py-12">
      ...
      >
        <div className={`inline-flex items-center gap-4 px-6 py-4 rounded-full text-2xl sm:text-4xl md:text-5xl font-black uppercase tracking-wider mb-6 ${darkMode ? `bg-${theme.primary.split('-')[1]}-900/30 text-${theme.primary.split('-')[1]}-400` : `bg-${theme.primary.split('-')[1]}-100 text-${theme.primary.split('-')[1]}-600`}`}>
          <Sparkles className="w-8 h-8 sm:w-10 sm:h-10" />
          Invest in Yourself
        </div>
        <h1 className={`text-lg sm:text-xl md:text-2xl font-bold mb-4 sm:mb-6 ${darkMode ? "text-slate-300" : "text-slate-700"}`}>
          Choose Your Plan
        </h1>
        <p className={`text-sm sm:text-lg md:text-xl mb-6 sm:mb-8 max-w-2xl mx-auto ${darkMode ? "text-slate-400" : "text-slate-600"}`}>
          Start free. Upgrade when you're ready to unlock deeper insights into your life's journey.
        </p>

        {/* Trust Indicators */}
        <div className={`flex flex-wrap justify-center gap-2 sm:gap-4 md:gap-8 text-xs sm:text-sm font-medium ${darkMode ? "text-slate-400" : "text-slate-600"}`}>
      ...
      <div className="flex justify-center mb-8">
        <div className={`inline-flex items-center p-1 rounded-full ${darkMode ? 'bg-white/5' : 'bg-slate-100'}`}>
          <button
        ...
        {tiers.map((tier, index) => {
          ...
          return (
            <motion.div
              ...
              className={`relative rounded-2xl sm:rounded-3xl p-4 sm:p-8 flex flex-col backdrop-blur-xl ${isHighlighted
                ? darkMode
                ...
                : isPro
                  ? darkMode
                    ? `bg-gradient-to-b from-white/5 to-white/[0.02] border border-${theme.primary.split('-')[1]}-500/20 shadow-xl shadow-${theme.primary.split('-')[1]}-500/5`
                    : `bg-gradient-to-b from-white/70 to-white/40 border border-${theme.primary.split('-')[1]}-200/40 shadow-xl shadow-${theme.primary.split('-')[1]}-500/5`
                  : darkMode
            ...
            >
              ...
              {isCurrentPlan && (
                <div
                  className="absolute -top-3 right-4 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider shadow-lg bg-gradient-to-r from-green-500 to-emerald-500 text-white border border-white/20"
                >
                  ✓ Current
                </div>
              )}
              ...
              {/* Header */}
              <div className="mb-8">
                <div className={`inline-flex p-4 rounded-2xl mb-6 backdrop-blur-md ${isPro || isHighlighted
                  ? `bg-gradient-to-br ${theme.primary}/20 shadow-lg shadow-${theme.primary.split('-')[1]}-500/10 border border-${theme.primary.split('-')[1]}-300/20`
                  : darkMode ? "bg-white/5 border border-white/5" : "bg-white/30 border border-white/20"
                  }`}>
                  <Icon className={`w-7 h-7 ${isPro || isHighlighted ? `text-${theme.primary.split('-')[1]}-500` : darkMode ? "text-slate-400" : "text-slate-600"}`} />
                </div>
              ...
              <div className="mt-auto">
                <motion.button
                  ...
                  className={`w-full py-4 px-6 rounded-2xl font-bold text-sm tracking-wide transition-all duration-300 backdrop-blur-md flex items-center justify-center gap-2 ${isCurrentPlan
